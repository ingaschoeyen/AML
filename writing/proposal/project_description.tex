...
We will be using Optical Character Recognition (OCR) methods to extract this text. As document quality is expected to differ aim to evaluate and improve data quality.
...
A potential extension could be also embedding the query with something like BGE-M3 to improve results using semantic search, this will depend on the performance that we measure.

Once quality output can be guaranteed we will design the front end in careful collaboration with the end users.
Whether the best solution would look like a search engine, or perhaps a chatbot, we can not say right now.
